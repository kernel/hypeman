...
func TestVZForkFromStandbyNetwork(t *testing.T) {
	if runtime.GOOS != "darwin" {
		t.Skip("vz tests require macOS")
	}
	if runtime.GOARCH != "arm64" {
		t.Skip("vz standby fork requires Apple Silicon (arm64)")
	}
	if !isMacOS14OrLater(t) {
		t.Skip("vz standby fork requires macOS 14+")
	}
	ensureMkfsExt4Available(t)
	...
	sourceBeforeStandby, err := mgr.GetInstance(ctx, sourceID)
	require.NoError(t, err)
	require.NotEmpty(t, sourceBeforeStandby.IP)
	require.NotEmpty(t, sourceBeforeStandby.MAC)

	sourceStandby, err := mgr.StandbyInstance(ctx, sourceID)
	if err != nil {
		dumpVZShimLogs(t, tmpDir)
		require.NoError(t, err)
	}
	require.Equal(t, StateStandby, sourceStandby.State)
	require.True(t, sourceStandby.HasSnapshot)

	forked, err := mgr.ForkInstance(ctx, sourceID, ForkInstanceRequest{
		Name:        "test-vz-fork-standby-copy",
	...
	require.True(t, forked.HasSnapshot)
	forkID := forked.Id
	t.Cleanup(func() { _ = mgr.DeleteInstance(context.Background(), forkID) })

	forked, err = mgr.RestoreInstance(ctx, forkID)
	if err != nil {
		dumpVZShimLogs(t, tmpDir)
		require.NoError(t, err)
	}
	...
	sourceRestored, err := mgr.RestoreInstance(ctx, sourceID)
	if err != nil {
		dumpVZShimLogs(t, tmpDir)
		require.NoError(t, err)
	}
	require.Equal(t, StateRunning, sourceRestored.State)
	require.False(t, sourceRestored.HasSnapshot)

	require.NoError(t, waitForExecAgent(ctx, mgr, sourceID, 30*time.Second))
	require.NoError(t, waitForExecAgent(ctx, mgr, forkID, 30*time.Second))

	sourceRestored, err = mgr.GetInstance(ctx, sourceID)
	require.NoError(t, err)
	forked, err = mgr.GetInstance(ctx, forkID)
	require.NoError(t, err)

	require.NotEmpty(t, sourceRestored.IP)
	require.NotEmpty(t, sourceRestored.MAC)
	require.NotEmpty(t, forked.IP)
	require.NotEmpty(t, forked.MAC)
	assert.NotEqual(t, sourceRestored.IP, forked.IP)
	assert.NotEqual(t, sourceRestored.MAC, forked.MAC)

	output, exitCode, err := vzExecCommand(ctx, sourceRestored, "echo", "source-after-standby-fork")
	require.NoError(t, err)
	require.Equal(t, 0, exitCode)
	assert.Equal(t, "source-after-standby-fork", strings.TrimSpace(output))

	output, exitCode, err = vzExecCommand(ctx, forked, "echo", "fork-from-standby")
	require.NoError(t, err)
	require.Equal(t, 0, exitCode)
	assert.Equal(t, "fork-from-standby", strings.TrimSpace(output))
}
