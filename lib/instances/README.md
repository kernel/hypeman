...
`metadata.json` also carries controller-owned `AutoStandbyState` timestamps when that feature is enabled, so idle countdown state can survive Hypeman restarts.
