...
func TestVGPUReconcileLivenessUncertainMetric(t *testing.T) {
	t.Parallel()
	...
	m.recordVGPUReconcileLivenessUncertain(t.Context())

	var rm metricdata.ResourceMetrics
	require.NoError(t, reader.Collect(t.Context(), &rm))
	metricValue := findMetric(t, rm, "hypeman_instances_vgpu_reconcile_liveness_uncertain_total")
