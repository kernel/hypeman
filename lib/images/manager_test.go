...
func TestDeleteAndRecreateDuringBuildTail(t *testing.T) {
	...
	staleDetails, err := m.ociClient.extractOCIImageDetails(digestHex)
	require.NoError(t, err)
	require.ErrorIs(t, m.finalizeImage(staleRef, &pullResult{Metadata: staleDetails.Metadata}, 1, firstMeta.BuildID, ""), errStaleBuild)
	currentMeta, err = readMetadata(p, repo, digestHex)
	require.NoError(t, err)
...
func TestImportLocalImagePersistsLayerManifest(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, string(manifest.Config.MediaType), layers.Config.MediaType)
	assert.Equal(t, manifest.Config.Digest.String(), layers.Config.Digest)
	assert.Equal(t, manifest.Config.Size, layers.Config.Size)
