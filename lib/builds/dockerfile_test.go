...
`
	...
	if refs[0] != "golang:1.21" {
		t.Errorf("expected golang:1.21, got %s", refs[0])
	}
	if refs[1] != "alpine:3.21" {
		t.Errorf("expected alpine:3.21, got %s", refs[1])
	}
}
...
`
	...
	if refs[0] != "alpine:3.21" {
		t.Errorf("expected alpine:3.21, got %s", refs[0])
	}
}
...
`
	...
	if refs[0] != "node:20-alpine" {
		t.Errorf("expected node:20-alpine, got %s", refs[0])
	}
}
...
`
	...
	if refs[0] != "node:20" {
		t.Errorf("expected node:20, got %s", refs[0])
	}
}
