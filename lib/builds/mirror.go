...
// Dockerfile and mirrors each one to the local registry. This allows the
// builder agent's rewriteDockerfileFROMs to find the images locally and
// rewrite FROM lines to pull from the local registry instead of Docker Hub.
//
// Individual mirror failures are logged but do not fail the build (graceful
// degradation — the builder will simply pull from upstream as before).
func (m *manager) mirrorBaseImagesForBuild(ctx context.Context, id string, req CreateBuildRequest) error {
	...
	m.logger.Info("mirroring base images for admin build", "id", id, "images", refs)
