...
Fork payload mailboxes use the same snapshot-memory handoff pattern for caller-provided JSON payloads. A guest-side component places a named marker plus token in memory before the standby snapshot is captured. When a fork is restored from that snapshot, Hypeman finds the named marker, patches the JSON payload, and optionally waits for the guest to send a UDP `stage=applied` acknowledgement for that mailbox name.
